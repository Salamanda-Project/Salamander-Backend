...
/**
 * Optimism Chain Adapter - Handles Optimism DEX interactions
 */
class OptimismAdapter {
  constructor() {
    this.networkName = 'optimism';
    this.marketModel = Market;
  }

  /**
   * Get all DEXes on Optimism network
  ...
  async getAllDexes() {
    ...
    try {
      ...
      logger.error('Failed to fetch DEXes from Optimism:', error);
      throw error;
    }
  }
  ...
  async getTradingPairsForDex(dexName, limit = 200) {
    ...
    try {
      ...
      logger.error(`Failed to fetch trading pairs for ${dexName} on Optimism:`, error);
      throw error;
    }
  }

  /**
   * Get active trading pairs from all Optimism DEXes
  ...
  async getActiveTradingPairs(limit = 20) {
    try {
      ...
      logger.error('Failed to fetch active trading pairs from Optimism:', error);
      throw error;
    }
  }
  ...
  async savePriceData(pairsData) {
    try {
      ...
      if (validPairs.length === 0) {
        logger.warn('No valid pairs to save for Optimism');
        return;
      }
      ...
      logger.info(`Saved ${validPairs.length} price records for Optimism`);
    } catch (error) {
      logger.error('Failed to save Optimism price data:', error);
      throw error;
    }
  }
  ...
  async updateAllPrices() {
    try {
      ...
      logger.error('Failed to update Optimism prices:', error);
      throw error;
    }
  }
}

module.exports = new OptimismAdapter();