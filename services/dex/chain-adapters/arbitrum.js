...
/**
 * Arbitrum Chain Adapter - Handles Arbitrum DEX interactions
 */
class ArbitrumAdapter {
  ...
  /**
   * Get all DEXes on Arbitrum network
  ...
  async getAllDexes() {
    ...
    try {
      ...
      logger.error('Failed to fetch DEXes from Arbitrum:', error);
      throw error;
    }
  }
  ...
  async getTradingPairsForDex(dexName, limit = 200) {
    ...
    try {
      ...
      logger.error(`Failed to fetch trading pairs for ${dexName} on Arbitrum:`, error);
      throw error;
    }
  }

  /**
   * Get active trading pairs from all Arbitrum DEXes
  ...
  async getActiveTradingPairs(limit = 20) {
    try {
      ...
      logger.error('Failed to fetch active trading pairs from Arbitrum:', error);
      throw error;
    }
  }
  ...
  async savePriceData(pairsData) {
    try {
      ...
      if (validPairs.length === 0) {
        logger.warn('No valid pairs to save for Arbitrum');
        return;
      }
      ...
      logger.info(`Saved ${validPairs.length} price records for Arbitrum`);
    } catch (error) {
      logger.error('Failed to save Arbitrum price data:', error);
      throw error;
    }
  }
  ...
  async updateAllPrices() {
    try {
      ...
      logger.error('Failed to update Arbitrum prices:', error);
      throw error;
    }
  }
}

module.exports = new ArbitrumAdapter();