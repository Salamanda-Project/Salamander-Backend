...
class BNBAdapter {
  ...
  async getAllDexes() {
    ...
    try {
      ...
      logger.error('Failed to fetch DEXes from BNB Chain:', error);
      throw error;
    }
  }
  ...
  async getTradingPairsForDex(dexName, limit = 200) {
    ...
    try {
      ...
      logger.error(`Failed to fetch trading pairs for ${dexName} on BNB Chain:`, error);
      throw error;
    }
  }

  /**
   * Get active trading pairs from all BNB DEXes
  ...
  async getActiveTradingPairs(limit = 20) {
    try {
      ...
      logger.error('Failed to fetch active trading pairs from BNB Chain:', error);
      throw error;
    }
  }
  ...
  async savePriceData(pairsData) {
    try {
      ...
      if (validPairs.length === 0) {
        logger.warn('No valid pairs to save for BNB Chain');
        return;
      }
      ...
      logger.info(`Saved ${validPairs.length} price records for BNB Chain`);
    } catch (error) {
      logger.error('Failed to save BNB Chain price data:', error);
      throw error;
    }
  }
  ...
  async updateAllPrices() {
    try {
      ...
      logger.error('Failed to update BNB Chain prices:', error);
      throw error;
    }
  }
}

module.exports = new BNBAdapter();